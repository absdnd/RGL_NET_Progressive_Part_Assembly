...
class Network(nn.Module):
    ...
    """
            ...
            Output: *, *  (two lists)
    """
    def linear_assignment_missing(self, pts1, pts2, centers1, quats1, centers2, quats2):
        import random
        pts_to_select = torch.tensor(random.sample([i for i  in range(1000)],100))
        pts1 = pts1[:,pts_to_select] 
        pts2 = pts2[:,pts_to_select]

        cur_part_cnt = pts1.shape[0]
        num_point = pts1.shape[1]

        with torch.no_grad():
            # Updating points with quat1 and quat2. 
            cur_quats1 = quats1.unsqueeze(1).repeat(1, num_point, 1)
            cur_centers1 = centers1.unsqueeze(1).repeat(1, num_point, 1)
            cur_pts1 = qrot(cur_quats1, pts1) + cur_centers1

            cur_quats2 = quats2.unsqueeze(1).repeat(1, num_point, 1)
            cur_centers2 = centers2.unsqueeze(1).repeat(1, num_point, 1)
            cur_pts2 = qrot(cur_quats2, pts2) + cur_centers2
